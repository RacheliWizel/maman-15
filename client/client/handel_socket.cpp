...
#include "handel_socket.h"

using boost::asio::ip::tcp;
using boost::asio::io_context;

HandleSocket::HandleSocket() : _ioContext(nullptr), _resolver(nullptr), _socket(nullptr), _connected(false)
{
...
bool HandleSocket::setSocketInfo(const std::string& address, const std::string& port)
{
 ...
 */
bool HandleSocket::validateIP(const std::string& address)
{
 ...
 */
bool HandleSocket::validatePort(const std::string& port)
{
	try
	{
 ...
 */
bool HandleSocket::connect()
{
	if (!validateIP(_address) || !validatePort(_port))
		return false;
	try
	{
 ...
 */
bool HandleSocket::receive(uint8_t* const buffer, const size_t size) const
{
 ...
 */
bool HandleSocket::send(const uint8_t* const buffer, const size_t size) const
{
	if (_socket == nullptr || !_connected || buffer == nullptr || size == 0)
		return false;
 ...
 */
bool HandleSocket::sendReceive(const uint8_t* const toSend, const size_t size, uint8_t* const response, const size_t resSize)
{
	if (!connect())
	{
		return false;
 ...
 * Handle Endianness.
 */
void HandleSocket::swapBytes(uint8_t* const buffer, size_t size) const
{
